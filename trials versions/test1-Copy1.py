...
class NeuralNetwork(object):
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        inputs = np.array(X, ndmin=2).T
        delta_weights_i_h += np.dot((hidden_error.T * hidden_grad), inputs.T) 
        
        #np.dot(hidden_error_term.T, X)
        
        #np.dot(X[:, None], hidden_error_term[:, None].T)
        
        # Weight step (hidden to output)        
        delta_weights_h_o += error * hidden_outputs.T 
        
        #np.dot(hidden_outputs.T[:, None], error[:, None])
